...
async function writeCardsAsRows(cards) {
  ...
  for (const card of cards) {
    ...
    if (idx >= 0) {
      // ✅ ensure numeric write
      priceByRow[idx] = toNumOrBlank(card.price);
    } else {
      minimalRowsToAppend.push([card.name ?? "", card.edition ?? ""]); // A:B only
      pricesForNewRows.push(toNumOrBlank(card.price)); // ✅ numeric or blank
    }
  }

  // build a true Date object for Seattle time
const now = new Date();
const seattleNow = new Date(
  now.toLocaleString("en-US", { timeZone: "America/Los_Angeles" })
);

// convert JS Date → Sheets serial (days since 1899-12-30)
...
// also force the number format to date-time
await sheets.spreadsheets.batchUpdate({
  spreadsheetId: SPREADSHEET_ID,
  requestBody: {
    requests: [{
  ...
  if (dataRows.length > 0) {
    await sheets.spreadsheets.values.update({
      ...
      requestBody: { values: priceByRow.map(v => [v]) }, // numbers go in as numbers
    });
  }
  ...
  if (minimalRowsToAppend.length > 0) {
    ...
    const tsValues = Array.from({ length: newCount }, (_, i) => [pricesForNewRows[i]]); // ✅ numeric (or "")
